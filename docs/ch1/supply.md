...
與需求表類似，最簡潔呈現供給的方式就是如表 (\@ref(tab:supply-schedule)) 所示，稱為**供給表** (supply schedule)。
...
以前述兩家廠商為例，若廠商A的供給函數為 $Q_A^s = \max(0, p - 5)$，廠商B的供給函數為 $Q_B^s = \max(0, 0.5p - 10)$，則市場供給函數為：

$$
Q^S = Q_A^s + Q_B^s = \max(0, p - 5) + \max(0, 0.5p - 10)
$$
